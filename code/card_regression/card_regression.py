...
from model import ConditionalGuidedModel, DeterministicFeedForwardNeuralNetwork, EarlyStopping, EMA
...
class Diffusion(object):
    def __init__(self, args, config, device=None):
        ...
        if config.diffusion.conditioning_signal == "OLS":
            ...
            self.aux_cost_function = nn.L1Loss()  # nn.L1Loss()  # nn.MSELoss()
        else:
            pass
    ...
    def evaluate_guidance_model(self, dataset_object, dataset_loader):
        ...
        for xy_0 in dataset_loader:
            ...
            y_batch_pred_mean = (
                self.compute_guiding_prediction(x_batch, method=self.config.diffusion.conditioning_signal)
                .cpu()
                .detach()
                .numpy()
            )
    ...
    def evaluate_guidance_model_on_both_train_and_test_set(
        ...
        logging.info(
            ...
            ).format(self.config.diffusion.conditioning_signal, y_train_rmse_aux_model, y_test_rmse_aux_model)
        )
    ...
    def make_subplot_at_timestep_t(self, cur_t, cur_y, y_i, y_0, axs, ax_idx, prior=False, testing=True):
        ...
        if prior:
            axs[ax_idx].set_title("$p({y}_\mathbf{prior})$", fontsize=23)
            axs[ax_idx].set_title("$p({y}_\mathbf{prior})$\n" + kl_unnorm_str, fontsize=23)
            axs[ax_idx].legend()
        else:
            axs[ax_idx].set_title("$p(\mathbf{y}_{" + str(cur_t) + "})$", fontsize=23)
        ...
        if not self.args.train_guidance_only:
            ...
            for epoch in range(start_epoch, self.config.training.n_epochs):
                ...
                for i, xy_0 in enumerate(train_loader):
                    ...
                    y_t_batch = q_sample(
                        y_batch, y_T_mean, self.alphas_bar_sqrt, self.one_minus_alphas_bar_sqrt, t, noise=e
                    )
                    ...
                    if step % self.config.training.validation_freq == 0 or step == 1:
                        if config.data.dataset in (
                            ...
                            with torch.no_grad():
                                ...
                                cur_y, y_i = self.obtain_true_and_pred_y_t(cur_t, y_p_seq, y_T_mean, y_batch)
                                self.make_subplot_at_timestep_t(
                                    cur_t, cur_y, y_i, y_batch, axs, self.num_figs, prior=True, testing=False
                                )
    ...
    def test(self):
        ...
        def store_gen_y_at_step_t(config, current_batch_size, idx, y_tile_seq, gen_y_by_batch_list):
            ...
            gen_y = (
                y_tile_seq[idx]
                .reshape(current_batch_size, config.testing.n_z_samples, config.model.y_dim)
                .cpu()
                .numpy()
            )
        ...
        def set_NLL_global_precision(test_var=True):
            if test_var:
                ...
                y_test_unnorm = (
                    y_test_unnorm if type(y_test_unnorm) is torch.Tensor else torch.from_numpy(y_test_unnorm)
                )
        ...
        if getattr(self.config.testing, "ckpt_id", None) is None:
            states = torch.load(os.path.join(log_path, "ckpt.pth"), map_location=self.device)
            ckpt_id = "last"
        else:
            states = torch.load(
                os.path.join(log_path, f"ckpt_{self.config.testing.ckpt_id}.pth"), map_location=self.device
            )
        ...
        # load auxiliary model
        if config.diffusion.conditioning_signal == "NN":
            aux_states = torch.load(os.path.join(log_path, "aux_ckpt.pth"), map_location=self.device)
        ...
        if config.data.dataset in ("uci", "renewable"):
        ...
        with torch.no_grad():
            ...
            self.make_subplot_at_timestep_t(cur_t, cur_y, y_i, y_check, axs, 0, testing=False)
            # plot at vis_step interval
            for i in range(1, self.num_figs):
                cur_t = i * self.vis_step
                cur_y, y_i = self.obtain_true_and_pred_y_t(cur_t, y_p_seq, y_T_mean_check, y_check)
                self.make_subplot_at_timestep_t(cur_t, cur_y, y_i, y_check, axs, i, testing=False)
            # plot at timestep T
            cur_t = self.num_timesteps
            cur_y, y_i = self.obtain_true_and_pred_y_t(cur_t, y_p_seq, y_T_mean_check, y_check)
            self.make_subplot_at_timestep_t(
        ...
        if config.testing.make_plot:
            assert config.data.dataset not in ("uci", "renewable")
            ...
            if config.data.no_multimodality:
                if not config.data.inverse_xy:
                    coverage, CI_y_pred, low, high = compute_PICP(
                        config=config, y_true=y_true, all_gen_y=all_gen_y, return_CI=True
                    )
            ...
            if dataset_object.normalize_y:
                ...
                all_gen_y = all_gen_y.reshape(
                    dataset_object.test_n_samples * config.testing.n_z_samples, config.model.y_dim
                )
                all_gen_y = (
                    dataset_object.scaler_y.inverse_transform(all_gen_y)
                    .astype(np.float32)
                    .reshape(dataset_object.test_n_samples, config.testing.n_z_samples, config.model.y_dim)
                )
            ...
            if config.data.inverse_xy:
                x_noiseless_mean = compute_y_noiseless_mean(
                    dataset_object, torch.from_numpy(all_true_y), config.data.true_function
                )
            ...
            if config.data.no_multimodality:
                ...
                if not config.data.inverse_xy:
                    ...
                    y_noiseless_mean = compute_y_noiseless_mean(
                        dataset_object, torch.from_numpy(all_true_x), config.data.true_function
                    )
                    logging.info(
                        (
            ...
            if config.testing.plot_gen:
                ...
                samp_idx = np.random.randint(
                    ...
                    size=(dataset_object.test_n_samples, n_samples_gen_y_for_plot, config.model.y_dim),
                )
                all_gen_y_ = np.take_along_axis(all_gen_y, indices=samp_idx, axis=1)
                if len(all_gen_y_.shape) == 3:
                    all_gen_y_ = all_gen_y_.reshape(
                        dataset_object.test_n_samples * n_samples_gen_y_for_plot, config.model.y_dim
                    )
                n_total_samples = all_true_x_tile.shape[0]
                gen_sampled_idx = np.random.choice(
                    np.arange(n_total_samples),
                    size=n_total_samples // (n_true_x_for_plot_scale * n_samples_gen_y_for_plot),
                    replace=False,
                )
                ax1.scatter(
